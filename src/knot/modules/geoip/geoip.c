...
static int geoip_additional(int state, knot_pkt_t *pkt, struct query_data *qdata, void *_ctx)
{
	assert(pkt);
	assert(qdata);
	assert(_ctx);
...
int geoip_load(struct query_module *self)
{
	assert(self);
	...
	query_module_step(self, QPLAN_ADDITIONAL, geoip_additional);
...
void geoip_unload(struct query_module *self)
{
	assert(self);
